...
async function main(){ const url = process.env.MP_URL || 'ws://localhost:8081'; const matchId = 'TESTE123'; const p1 = await connect(url); const p2 = await connect(url);
  ...
  const set1 = { type:'action', matchId, playerId:'p1', actionId:'l1', actionType:'SET_LEADER', payload:{ side:'p1', leader:{ key:'katsu', name:'Katsu, o Vingador' } } };
  p1.send(JSON.stringify(set1));
  await new Promise(r=>setTimeout(r,200));
  const set2 = { type:'action', matchId, playerId:'p2', actionId:'l2', actionType:'SET_LEADER', payload:{ side:'p2', leader:{ key:'ademais', name:'Ademais, Aranha Negra' } } };
  p2.send(JSON.stringify(set2));
  await new Promise(r=>setTimeout(r,300));
