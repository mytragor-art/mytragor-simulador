...
wss.on('connection', (ws, req) => {
  ...
  const room = rawRoom ? rawRoom : null;
  ...
  ws.on('message', data => {
    ...
    try{
      if(typeof data === 'string'){
          ...
          if(obj && obj.type === 'join' && obj.room){ ws.room = (''+obj.room).toUpperCase(); console.log('join message assigned room ->', ws.room); broadcastRooms(); }
          // Some clients send a first message with { room: 'XXX' } (legacy). Handle that too.
          if(obj && obj.room && !ws.room){ ws.room = (''+obj.room).toUpperCase(); console.log('assigned room from join message ->', ws.room); broadcastRooms(); }
